...
variable "consul_version" {
  ...
  default     = "2.0.3" # renovate: datasource=github-releases depName=hashicorp/consul extractVersion=v(?<version>.+)
}

variable "nomad_version" {
  description = "Nomad version used for the environment"
  default     = "2.0.4" # renovate: datasource=github-releases depName=hashicorp/nomad extractVersion=v(?<version>.+)
}
