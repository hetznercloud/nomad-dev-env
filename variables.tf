...
variable "consul_version" {
  ...
  default     = "1.22.7" # renovate: datasource=github-releases depName=hashicorp/consul extractVersion=v(?<version>.+)
}

variable "nomad_version" {
  description = "Nomad version used for the environment"
  default     = "1.11.3" # renovate: datasource=github-releases depName=hashicorp/nomad extractVersion=v(?<version>.+)
}
